...
`omc team` runtime-v2 is gaining an opt-in worker worktree mode. Worktree-backed workers run from dedicated git worktrees while task lifecycle, mailbox, status, and manifest files stay under the leader workspace's canonical `.omc/state` root.

### Contract

- Worktree paths use `<repo>/.omc/team/<team-name>/worktrees/<worker-name>`.
- `OMC_TEAM_STATE_ROOT` points workers back to `<repo>/.omc/state`.
